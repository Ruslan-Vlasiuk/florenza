  ...
  if (action === 'pay') {
    ...
    await sendTelegramMessageWithButtons(
      ...
      [
        ...
        '',
        'Натисніть кнопку нижче — відкриється безпечна сторінка Monobank. Apple Pay / Google Pay / картка.',
        '',
        `Якщо кнопка не спрацювала — відкрийте посилання вручну:`,
        `<a href="${result.url}">${result.url}</a>`,
        '',
        '<i>Якщо сторінка не завантажується — натисніть «⋮» у Telegram і виберіть «Відкрити в Safari/Chrome». Mono пейдж не дружить з in-app браузерами.</i>',
        '',
